...
func NewBorderLayout(top, bottom, left, right Node) LayoutManager {
	l := &BorderLayout{top, bottom, left, right, 0.0} //LayoutProps.Size(Padding)}
	return l
}

func (l *BorderLayout) Layout(childList *list.List, size geom.Point) {
	var topSize, bottomSize, leftSize, rightSize geom.Point
	var topHeight, bottomHeight, leftWidth, rightWidth float64
	...
	topSize = geom.Point{size.X, topHeight}
	bottomSize = geom.Point{size.X - leftWidth - rightWidth, bottomHeight}
	leftSize = geom.Point{leftWidth, size.Y - topHeight - bottomHeight}
	rightSize = geom.Point{rightWidth, size.Y - topHeight - bottomHeight}

	if l.top != nil && l.top.Visible() {
		l.top.SetSize(topSize)
		l.top.SetPos(geom.Point{0, 0})
	}
	if l.bottom != nil && l.bottom.Visible() {
		l.bottom.SetSize(bottomSize)
		l.bottom.SetPos(geom.Point{leftSize.X, size.Y - bottomSize.Y})
	}
	if l.left != nil && l.left.Visible() {
		l.left.SetSize(leftSize)
		l.left.SetPos(geom.Point{0, topSize.Y})
	}
	if l.right != nil && l.right.Visible() {
		l.right.SetSize(rightSize)
		l.right.SetPos(geom.Point{size.X - rightSize.X, topSize.Y})
	}
