...
func (f *FourierThingy) Paint(gc *gg.Context) {
    ...
    //gc.DrawPoint(0.0, 0.0, 6.0)
    //gc.SetFillColor(colornames.YellowGreen)
    //gc.Fill()
}
...
func (p *FourierPen) Paint(gc *gg.Context) {
    ...
    gc.SetFillColor(colornames.OrangeRed)
...
func main() {
    ...
    panel.SetColor(colornames.DarkSlateGrey.Dark(0.5))
    panel.SetBorderWidth(1.0)
    panel.SetBorderColor(colornames.WhiteSmoke)
	root.Add(panel)

    label := adagui.NewLabel("Draw something, but use only one stroke!")
    label.SetPos(panel.Bounds().SW().SubXY(0, label.Size().Y))
    label.SetSize(geom.Point{panel.Size().X, label.Size().Y})
    panel.Add(label)
    ...
    panel.SetColor(colornames.DarkSlateBlue.Dark(0.5))
    panel.SetBorderWidth(1.0)
    panel.SetBorderColor(colornames.WhiteSmoke)
    root.Add(panel)
    ...
    label.SetPos(panel.Bounds().C().Sub(label.Size().Mul(0.5)))
    panel.Add(label)
    ...
    label = adagui.NewLabel("Tap somewhre on the empty screen to start.")
    label.SetPos(panel.Bounds().SW().SubXY(0, label.Size().Y))
    label.SetSize(geom.Point{panel.Size().X, label.Size().Y})
    panel.Add(label)
