...
//		{"Type", Keyboard, nil},
		{"Sliders", SlidersAndScrollbars, nil},
...
// SlidersAndScrollbars
func SlidersAndScrollbars(size geom.Point) adagui.Node {
	root := adagui.NewGroup()
	root.Layout = adagui.NewPaddedLayout()
	root.SetMinSize(size)
    ...
    scrVal1 := binding.NewFloat()
    scrStr1 := binding.FloatToStringWithFormat(scrVal1, "%.3f")
    scrVal2 := binding.NewFloat()
    scrStr2 := binding.FloatToStringWithFormat(scrVal2, "%.3f")
    scrVal3 := binding.NewFloat()
    scrStr3 := binding.FloatToStringWithFormat(scrVal3, "%.3f")
    scrVal4 := binding.NewFloat()
    scrStr4 := binding.FloatToStringWithFormat(scrVal4, "%.3f")

	scr := adagui.NewScrollbarWithData(160.0, adagui.Horizontal, scrVal1)
	scr.SetVisiRange(0.1)
	panel.Add(scr)

	scr = adagui.NewScrollbarWithData(160.0, adagui.Horizontal, scrVal2)
	scr.SetVisiRange(0.3)
	panel.Add(scr)

	scr = adagui.NewScrollbarWithData(160.0, adagui.Horizontal, scrVal3)
	scr.SetVisiRange(0.7)
	panel.Add(scr)

	scr = adagui.NewScrollbarWithData(160.0, adagui.Horizontal, scrVal4)
	scr.SetVisiRange(1.0)
	panel.Add(scr)

	row := adagui.NewGroupPL(panel, adagui.NewHBoxLayout())
    lbl1 := adagui.NewLabelWithData(scrStr1)
    lbl2 := adagui.NewLabelWithData(scrStr2)
    lbl3 := adagui.NewLabelWithData(scrStr3)
    lbl4 := adagui.NewLabelWithData(scrStr4)
	row.Add(lbl1, adagui.NewSpacer(), lbl2, adagui.NewSpacer(), lbl3,
        adagui.NewSpacer(), lbl4)
